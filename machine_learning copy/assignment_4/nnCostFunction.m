...
%J = sum(sum(( (-1 * y' * log(h_func') - ((1 - y') * log(1 -h_func')))/m))) + ((lambda/(2*m)) * (sum(sum(Theta1.^2)) + %sum(sum(Theta2.^2)));

J = (-1/m) * sum(sum(( y'.* log(h_func)) + ((1 - y').* log(1 - h_func)))) + ((lambda / (2 * m)) * (sum(sum((Theta1(:, 2:end)).^2)) + sum(sum((Theta2(:, 2:end)).^2))));
